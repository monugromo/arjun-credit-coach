...
import novioCardArt from "@/assets/sbm-novio-front.webp.asset.json";
...
function QuestionPage({ state, update }: { state: LoanJourneyState; update: (patch: Partial<LoanJourneyState>) => void }) {
  ...
  return <div key={state.step} className="flex min-h-0 flex-1 flex-col bg-card motion-safe:animate-[loan-page-slide_260ms_ease-out]"><div className="flex-1 overflow-y-auto px-5 pb-5 pt-6"><h2 className="font-display text-[22px] font-bold leading-[28px] text-foreground">Add your details for best offers</h2>{state.step === "details" ? <div className="mt-6 space-y-4"><InlineSelect label="Select Employment type" value={state.work} options={[{ label: "Salaried" }, { label: "Self-employed" }, { label: "Student" }, { label: "Other" }]} onChange={(work) => update({ work: work as WorkType, salaryMode: work === "Salaried" ? state.salaryMode : "" })} />{state.work === "Salaried" && <InlineSelect label="How do you get paid?" value={state.salaryMode} options={[{ label: "Bank transfer" }, { label: "Cash" }, { label: "Cheque" }]} onChange={(salaryMode) => update({ salaryMode: salaryMode as SalaryMode })} />}<AmountField id="monthly-income" label="New monthly income" value={state.income} onChange={(income) => update({ income })} /><AmountField id="loan-amount" label="Loan amount" value={state.loanAmount} onChange={(loanAmount) => update({ loanAmount })} /></div> : <div className="mt-6 space-y-5"><FormField label="Pincode"><PincodeInput value={state.pincode} onChange={(pincode) => update({ pincode })} /></FormField><DateWheel value={state.dob} onChange={(dob) => update({ dob })} /></div>}</div><div className="shrink-0 bg-card px-5 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-3"><Button type="button" disabled={state.step === "details" ? !detailsValid : !locationValid} onClick={() => update({ step: state.step === "details" ? "location" : "checking" })} className="mx-auto h-[54px] w-full max-w-[361px] rounded-lg bg-primary-deep text-base font-semibold text-primary-foreground hover:bg-primary-deep/90">Continue</Button></div></div>;
}
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className="relative bg-primary-deep/5 px-4 pb-4 pt-4">
        ...
        <div className="pointer-events-none absolute right-0 top-0 h-28 w-40">
          <img src={novioCardArt.url} alt="SBM Novio Credit Card" loading="lazy" width={768} height={1217} className="absolute right-0 top-0 w-20 origin-bottom-right rotate-45 rounded-lg drop-shadow-lg" />
        </div>
        <p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Why this fits you</p>
        <div className="mt-2 flex flex-wrap gap-2">
...
function UnlockIssueSheet({ offer, onClose, onFix }: { offer: LockedOffer; onClose: () => void; onFix: () => void }) {
  return <Sheet title="Why this loan is locked" onClose={onClose}><div className="mt-5"><div className="flex items-center gap-3 rounded-lg border border-border bg-card p-3 shadow-sm"><LenderLogo name={offer.lender} logo={offer.logo} size="sm" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-foreground">{offer.lender}</p><p className="text-xs text-muted-foreground">{offer.product}</p></div><LockKeyhole className="h-4 w-4 shrink-0 text-muted-foreground" /></div><div className="mt-4 rounded-lg bg-muted/40 px-4 py-4"><p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Issue found in your report</p><h4 className="font-display mt-1.5 text-base font-bold leading-snug text-foreground">{offer.issue}</h4><p className="mt-2 text-sm leading-5 text-muted-foreground">{offer.explanation}</p></div><Button onClick={onFix} className="mt-6 h-[54px] w-full rounded-lg bg-primary-deep text-base font-bold text-primary-foreground hover:bg-primary-deep/90">Fix this with Arjun</Button><p className="mt-3 text-center text-xs text-muted-foreground">This does not affect your credit score - no enquiry was made.</p></div></Sheet>;
}
    ...
    <div className="relative flex min-h-0 flex-1 flex-col bg-card motion-safe:animate-[loan-page-slide_260ms_ease-out]">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 pb-6 pt-5">
        {noBureau ? <>
          ...
          {available.length > 0 ? <section><div className="space-y-3">{visibleAvailable.map((offer, index) => <OfferCard key={offer.id} featured={index === 0} offer={offer} leadStatus={state.applied[offer.id]} notice={applyErrors[offer.id]} onApply={() => openApply(offer)} />)}</div>{available.length > 4 && <button type="button" onClick={() => setShowAll(!showAll)} className="mt-3 flex w-full items-center justify-center gap-1 text-sm font-semibold text-foreground hover:text-foreground/80">{showAll ? <>View less<ChevronUp className="h-4 w-4" /></> : <>View more<ChevronDown className="h-4 w-4" /></>}</button>}</section> : <div className="rounded-lg border border-border bg-card p-5 shadow-sm"><h3 className="font-display text-lg font-bold text-foreground">Nothing available right now</h3><p className="mt-1 text-sm text-muted-foreground">Your credit profile needs a little work. See what you can unlock below.</p><Button onClick={() => onChat("apply")} className="mt-4 h-[54px] w-full rounded-lg bg-primary-deep text-base font-semibold text-primary-foreground shadow-none hover:bg-primary-deep/90">Talk to Arjun</Button></div>}

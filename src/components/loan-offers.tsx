...
function UnlockIssueSheet({ offer, onClose, onFix }: { offer: LockedOffer; onClose: () => void; onFix: () => void }) {
  return <Sheet title="Why this loan is locked" onClose={onClose}><div className="mt-5"><div className="flex items-center gap-2.5 rounded-full border border-border bg-muted/40 py-1.5 pl-1.5 pr-3"><LenderLogo name={offer.lender} logo={offer.logo} size="sm" /><p className="min-w-0 flex-1 truncate text-sm font-semibold text-foreground">{offer.lender} <span className="font-normal text-muted-foreground">· {offer.product}</span></p><LockKeyhole className="h-4 w-4 shrink-0 text-muted-foreground" /></div><div className="mt-5 rounded-lg bg-muted/40 px-4 py-3.5"><p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Issue found in your report</p><h4 className="font-display mt-1.5 text-base font-bold leading-snug text-foreground">{offer.issue}</h4></div><p className="mt-3 text-sm leading-5 text-muted-foreground">{offer.explanation}</p><Button onClick={onFix} className="mt-6 h-[54px] w-full rounded-lg bg-primary-deep text-base font-bold text-primary-foreground hover:bg-primary-deep/90">Fix this with Arjun</Button><p className="mt-3 text-center text-xs text-muted-foreground">This does not affect your credit score — no enquiry was made.</p></div></Sheet>;
}

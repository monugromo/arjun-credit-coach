...
function CreditCardEmptyState({ onChat }: { onChat: () => void }) {
  return <section className="rounded-lg border border-dashed border-border bg-card px-5 py-6 text-center shadow-sm"><div className="text-3xl leading-none">😔</div><p className="mt-2 text-3xl font-bold text-foreground" style={{ fontFamily: "Caveat, cursive" }}>Sorry!</p><h3 className="font-display mt-3 text-lg font-bold text-foreground">Credit builder launching soon</h3><Button variant="link" onClick={onChat} className="mt-2 h-auto p-0 text-sm font-semibold text-primary-deep">Talk to Arjun</Button></section>;
}
...
function UnlockIssueSheet({ offer, onClose, onFix }: { offer: LockedOffer; onClose: () => void; onFix: () => void }) {
  return <Sheet title="Why this loan is locked" onClose={onClose}><div className="mt-5"><div className="flex items-center gap-3 border-b border-border pb-4"><LenderLogo name={offer.lender} logo={offer.logo} /><div className="min-w-0 flex-1"><h4 className="font-display text-lg font-bold text-foreground">{offer.lender}</h4><p className="text-sm text-muted-foreground">{offer.product}</p></div><LockKeyhole className="h-5 w-5 shrink-0 text-muted-foreground" /></div><div className="py-5"><p className="text-xs font-bold uppercase text-muted-foreground">Issue found in your report</p><div className="mt-3"><h4 className="font-display text-lg font-bold text-foreground">{offer.issue}</h4><p className="mt-2 text-sm leading-5 text-muted-foreground">{offer.explanation}</p></div></div><Button onClick={onFix} className="h-[54px] w-full rounded-lg bg-primary-deep text-base font-bold text-primary-foreground hover:bg-primary-deep/90">Fix this with Arjun</Button><p className="mt-3 text-center text-xs text-muted-foreground">This does not affect your credit score — no enquiry was made.</p></div></Sheet>;
}
